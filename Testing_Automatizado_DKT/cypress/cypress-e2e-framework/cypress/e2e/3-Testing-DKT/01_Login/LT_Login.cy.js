describe('Test Suite - Login with user: RS (Store Manager), Store Organizer and Coach leader', () => {
  ...
  it('should visit a page and assert on its content', () => {
    ...
    cy.get("#username").should("be.visible").type(data.Users.username_LT)
    cy.get("#password").should("be.visible").type(data.Users.password_6)

    //Click in Sing on. 
    cy.get("#cnxbton").should("be.visible").click()
    cy.wait(1000)
    cy.log('Login with RS (Store Manager), Store Organizer and Coach leader ');
