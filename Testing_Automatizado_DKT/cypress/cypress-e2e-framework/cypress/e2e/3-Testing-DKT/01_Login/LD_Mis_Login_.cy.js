describe('Test Suite - Login with user: Sport leader and Missionary', () => {
  ...
  it('should visit a page and assert on its content', () => {
    ...
    cy.get("#username").should("be.visible").type(data.Users.username_LD_MIS)
    cy.get("#password").should("be.visible").type(data.Users.password_4)

    //Click in Sing on.
    cy.get("#cnxbton").should("be.visible").click()
    cy.wait(1000)
    cy.log('Login with Sport leader and Missionary ');
